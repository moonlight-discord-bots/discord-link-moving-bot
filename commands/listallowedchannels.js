...
exports.run = async (client, message) => {
  ...
  const list = message.guild.channels.filter(c=>c.type === 'text' && c.topic && c.topic.includes('{allowlinks}'))
  .map(d=>d.toString())
  .join('\n')
  ...
  .setFooter('To remove a channel from this list, simply remove {allowlinks} from the topic')
