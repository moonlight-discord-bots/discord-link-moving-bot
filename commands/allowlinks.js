exports.run = async (client, message, args) => {
  ...
  if(channel.topic.includes('{allowlinks}'))
    return message.channel.send(`:warning: Links in ${sameChannel ? 'this' : 'that'} channel are already allowed`)

  await channel.edit({
    topic: channel.topic + ' {allowlinks}'
  },
  `Allowing links: action taken by ${message.author.tag}`
  )

  message.channel.send(`:white_check_mark: Links are now allowed to be posted in ${sameChannel ? 'this' : 'that'} channel`)
